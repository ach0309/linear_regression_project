...
## Exploration
Exploration was started by checking the data and creating boxplots to explore outliers. The we checked the target feature which is area (in hectares), the area distribution is extremely skewed (right skewed), using the log function, we are able to make the distribution less skewed and stabilize variance.

We then went on to exploring the relationships between our target against numerical and non-numerical features, learning that we need to do encoding on _month_ feature as it shows some seasonal relationship that could be useful for fire ignition and predict overall burned area. 
...
We dropped duplicates, encoded _month_ feature, log transform our target _(area)_, and dropped features that are not very relevant.
...
We later converted the log-transformed area feature into hectares to see the difference.
